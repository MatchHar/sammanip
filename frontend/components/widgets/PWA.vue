...
const showPWA = () => {
    ...
    pwaInstall.externalPromptEvent = window.sammanInstallPromptEvent || pwaInstall.externalPromptEvent;
...
onMounted(() => {
    getBrowser();
    const pwaInstall = document.getElementsByTagName('pwa-install')[0];
    if (pwaInstall && window.sammanInstallPromptEvent) {
        pwaInstall.externalPromptEvent = window.sammanInstallPromptEvent;
    }
