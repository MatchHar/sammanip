...
describe('useShortcuts()', () => {
  it('loadShortcuts() registers the supported keymap on a private deployment', () => {
    ...
    assert.equal(distinctKeys.has('r'), false);
    assert.equal(distinctKeys.has('a'), false);
  });

  it('does not expose deployment-specific tool shortcuts', () => {
    const { keyMap } = loadAndGetKeyMap({ originalSite: true });
    assert.equal(keyMap.some((e) => e.keys === 'i'), false);
    assert.equal(keyMap.some((e) => e.keys === 'D'), false);
  });
