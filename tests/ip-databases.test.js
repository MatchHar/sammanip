...
describe('createInitialIpDBs()', () => {
  it('returns fresh array and fresh entries', () => {
    ...
    a[0].enabled = true;
    assert.equal(b[0].enabled, false, 'mutating one copy must not affect the other');
    assert.equal(IP_DATABASES[0].enabled, false, 'must not mutate the static definition');
  });
});
