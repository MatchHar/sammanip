...
describe('refererCheck — malformed inputs', () => {
  it('safely rejects non-URL strings without throwing', () => {
    assert.equal(refererCheck('not-a-url'), false);
  });

  it('rejects non-http schemes and credential-bearing URL values', () => {
    process.env.ALLOWED_DOMAINS = 'example.com';
    assert.equal(refererCheck('javascript:alert(1)'), false);
    assert.equal(refererCheck('https://user:pass@example.com/'), false);
  });

  it('trims and normalizes configured domains', () => {
    process.env.ALLOWED_DOMAINS = ' Example.COM , ';
    assert.equal(refererCheck('https://example.com/'), true);
  });
});
